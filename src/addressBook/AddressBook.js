...
class AddressBook extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevStateString !== updatedStateString) {
      ...
      // sessionStorage.setItem('contacts', updatedStateString)
    }
  }

  componentDidMount() {
    const savedSateFromLocalStorage = localStorage.getItem("contacts")

    if (savedSateFromLocalStorage) {
      this.setState({
        contacts: JSON.parse(savedSateFromLocalStorage)
      })
    }
  }
